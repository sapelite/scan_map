  ...
  return (
    <li className="flex items-center justify-between py-2 border-b border-border last:border-0">
      <div className="flex items-center gap-3 min-w-0">
        <span className={`status-dot ${f.ok ? "bg-[#2f9e44]" : "bg-[#d92d20]"}`} aria-hidden />
        ...
        <div className="p-7 space-y-6">
            ...
            <div className="flex items-start gap-6">
              <div className="shrink-0">
                {websiteStatus === "audited" ? (
                  <>
                    <div className={`text-5xl font-semibold tabular-nums tracking-tight ${score >= 70 ? "text-[#2f9e44]" : score >= 40 ? "text-[#d97706]" : "text-[#d92d20]"}`}>
                      {score}
                    </div>
                    <div className="text-xs text-muted-foreground mt-1">{lead.stats.riskLevel} risk</div>
                  </>
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                {websiteStatus === "audited" && (
                  <div className="label-section">Site audited</div>
                )}
                {websiteStatus === "unreachable" && (
                  <>
                    <div className="label-section">Site unreachable</div>
                    {lead.websiteFailReason && (
                      <div className="text-sm text-muted-foreground mt-0.5">{lead.websiteFailReason}</div>
                    )}
                    {externalUrl && (
                      <a
                        href={externalUrl}
                        ...
                        className="text-sm text-foreground hover:underline mt-2 inline-block break-all"
                      >
                        {externalUrl}
                      </a>
                    )}
                ...
                {websiteStatus === "none" && (
                  <div className="label-section">No website on Maps</div>
                )}
                <div className="flex flex-wrap items-center gap-2 mt-3">
                  <button onClick={reaudit} disabled={reauditing} className="btn btn-secondary btn-sm">
          ...
          {audit && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatTile
                label="Load time"
                value={audit.loadTimeMs > 0 ? `${(audit.loadTimeMs / 1000).toFixed(2)}s` : "n/a"}
                sub={audit.loadTimeMs > 0 ? (audit.loadTimeMs < 3000 ? "Fast" : "Slow") : undefined}
              />
              ...
              <StatTile
                ...
                sub={`${audit.internalLinks} internal, ${audit.externalLinks} external`}
              />
            </div>
          )}
            ...
            <div className="card p-5 space-y-3 text-sm">
              <Row label="Website" value={
                websiteStatus === "audited" && audit
                  ? <a href={audit.finalUrl} target="_blank" rel="noopener noreferrer" className="text-foreground hover:underline break-all block">{audit.finalUrl}</a>
                  : websiteStatus === "unreachable" && externalUrl
                  ? <a href={externalUrl} target="_blank" rel="noopener noreferrer" className="text-foreground hover:underline break-all block">{externalUrl}</a>
              ...
              <Row label="Maps" value={
                <a href={lead.mapsUrl} target="_blank" rel="noopener noreferrer" className="text-foreground hover:underline">Open in Google Maps</a>
              } />
            </div>
          </section>
              ...
              <div className="card p-5 grid grid-cols-2 md:grid-cols-3 gap-2">
                {Object.entries(audit.socials).map(([k, v]) => v ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-2 rounded-(--radius) bg-muted hover:bg-foreground hover:text-white transition-colors text-sm font-medium"
                  >
                    {SOCIAL_LABEL[k]}
                  </a>
                ) : null)}
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className={`btn btn-sm ${dirty && !saving ? "btn-primary" : "btn-secondary"}`}
              >
          ...
          {lead.auditAttempts && lead.auditAttempts.length > 0 && (
            <details className="card p-5 group">
              <summary className="cursor-pointer flex items-center justify-between list-none">
                ...
                <span className="text-xs text-muted-foreground">{lead.auditAttempts.length}</span>
              </summary>
              <div className="mt-3 space-y-1 font-mono text-[11px] leading-relaxed">
                {lead.auditAttempts.map((a, i) => {
                  const ok = / OK \(/.test(a);
                  const cls = ok ? "text-[#2f9e44]" : "text-muted-foreground";
                  return (
                    <div key={i} className={cls}>
                      <span className="text-(--tertiary-foreground) mr-2">{String(i + 1).padStart(2, "0")}</span>
                      {a}
                    </div>
                  );
                })}
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <button onClick={() => onQuickStrike(lead)} className="btn btn-primary">Email</button>
            <button onClick={() => onDownloadPDF(lead)} className="btn btn-secondary">Download PDF</button>
...
function Tag({ ok, label }: { ok: boolean; label: string }) {
  return (
    <div className={`flex items-center gap-2 text-sm ${ok ? "text-foreground" : "text-(--tertiary-foreground)"}`}>
      <span className={`status-dot ${ok ? "bg-[#2f9e44]" : "bg-[#d2d2d7]"}`} />
      {label}
    </div>
  );
}
